...
public partial class MainScript
{
    ...
    {
        //맵생성
        for (int i = 0; i < 8; i++) for (int j = 0; j < 8; j++) 맵[i, j] = new 구역();
    ...
    {
        맵[4, 5].아이템 = new 아이템형();
        맵[4, 5].아이템.오브젝트 = 임시아이템[0];
        맵[4, 5].아이템.오브젝트.transform.parent = MapObject.transform;
        맵[4, 5].게임오브젝트를구역에고정(맵[4, 5].아이템.오브젝트);
    }
    void 임시지형생성(int x, int y, GameObject tt)
    {
        맵[x, y].기물 = new 기물형();
        맵[x, y].기물.오브젝트 = Instantiate(tt);
        맵[x, y].기물.구역통과여부 = false;
        맵[x, y].기물.움직임여부 = false;

        맵[x, y].기물.오브젝트.transform.parent = MapObject.transform;
        맵[x, y].게임오브젝트를구역에고정(맵[x, y].기물.오브젝트);
    }
}
